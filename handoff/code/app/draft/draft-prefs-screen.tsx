"use client";
// ============================================================
// app/(app)/draft/draft-prefs-screen.tsx — DRAFT ORDER page
// ------------------------------------------------------------
// Worked port of the prototype's ReorderList + Grip + DraftPrefsScreen
// (reference/prototype-src/app/screens_draft.jsx). The full "Draft"
// page: drag-to-rank snake-draft POSITION preference, then drag-to-rank
// TEAM wishlist. Opened from the pre-draft Draft tab (standby-screen).
//
// MARKUP + the drag interaction (pointer events, touch-action:none).
// The ORDER STATE is local UI; persist it via your existing
// "save draft preferences" mutation in onPositionsChange / onTeamsChange
// (debounced upsert). Do not change how/where it's stored.
// ============================================================
import * as React from "react";
import { ScreenHeader, Scroll, CountryLogo } from "@/components/wc-ui";

/* ------------------------------------------------------------
   ReorderList — pointer/touch drag-to-rank with live reordering.
   `items` = array of stable string keys. renderRow(key, rank, handlers)
   must spread {...handlers} onto the drag handle (Grip).
------------------------------------------------------------ */
...
export function DraftPrefsScreen({
  ...
  playerCount?: number;
  teams: WishlistTeam[];          // default order ≈ FIFA ranking
  onClose?: () => void;
  ...
  // snake draft over 3 rounds: position p → overall picks p, (2N+1−p), (2N+p)
      ...
      <ScreenHeader
        ...
        right={
          <button
            type="button"
            onClick={onClose}
            className="h-9 px-[18px] rounded-pill bg-brand text-brand-on font-extrabold text-[13.5px] cursor-pointer border-0"
          >
            Done
          </button>
        }
          ...
          renderRow={(key, rank, h) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={
                    "w-[30px] text-center shrink-0 font-num font-bold text-[18px] " +
                    (top ? "text-brand-ink" : "text-ink-3")
                  }
                >
                  {rank + 1}
                </span>
