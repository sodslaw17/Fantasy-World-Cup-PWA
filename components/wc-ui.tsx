...
export { nameToColor } from "@/lib/name-color";
...
export function ScreenHeader({
  ...
  return (
    <div className="bg-brand-header pt-[calc(env(safe-area-inset-top)+8px)]" style={{ color: "var(--brand-header-on)" }}>
      {/* pl-5 left, pr-14 right — reserves 56px on the right for the fixed ThemeManager button */}
      <div className="flex items-end justify-between gap-3 pl-5 pr-14 pt-2.5 pb-3">
        <div className="min-w-0">
          {sub && (
            <div className="text-[13px] font-semibold mb-0.5 truncate" style={{ opacity: 0.72 }}>{sub}</div>
          )}
          <h1
            className={cx(
              "m-0 font-display font-bold uppercase tracking-[.005em] leading-none",
      ...
      <WaveStrip intensity={waveIntensity} brandTinted />
    </div>
  );
}
...
/* ============================================================
   ...
   ============================================================ */
export function Scroll({ children, pad = 14, bottomPad = 74 }: { children: React.ReactNode; pad?: number; bottomPad?: number }) {
  return (
    <div
      className="wc-scroll flex-1 overflow-y-auto flex flex-col gap-3 [-webkit-overflow-scrolling:touch] bg-brand-tint/55"
      style={{ padding: `12px ${pad}px calc(env(safe-area-inset-bottom) + ${bottomPad}px)` }}
    >
      {children}
    </div>
  );
...
export function CountryName({
  ...
  return (
    ...
    >
      ...
      <span className={"truncate" + (strikethrough ? " line-through" : "")}>{abbrev ? code : name}</span>
    </span>
  );
}
      ...
      {order.map((e) => {
        ...
        return (
          <div key={e.place} className="grid justify-items-center gap-1.5 flex-1 min-w-0">
            <div
              className={cx("grid place-items-center overflow-hidden text-white font-num font-bold shadow-md", radius, top ? "text-[18px]" : "text-[15px]")}
              style={{ width: av, height: av, background: e.color, border: `3px solid ${POD_COL[e.place]}` }}
            >
              {e.iconUrl ? (
                <img src={e.iconUrl} alt={e.name} className="w-full h-full object-cover" />
              ) : (
                e.initials
              )}
            </div>
...
export function TabBar({
  ...
  return (
    <div className="bg-brand-header border-t border-line flex justify-around pt-2 [box-shadow:0_-4px_18px_rgba(20,21,26,.08)] pb-[calc(env(safe-area-inset-bottom)+6px)]">
      {tabs.map((t) => {
          ...
          >
            <span
              className={cx(
                ...
                on ? "bg-brand shadow-sm" : "bg-transparent"
              )}
            >
              <span
                className="block w-[23px] h-[23px]"
                style={{ color: on ? "var(--brand-on)" : "var(--brand-header-on)", opacity: on ? 1 : 0.55 }}
              >
                {TAB_ICONS[t.icon]}
              </span>
            </span>
            <span
              className={cx("text-[10.5px] whitespace-nowrap", on ? "font-extrabold" : "font-semibold")}
              style={{ color: "var(--brand-header-on)", opacity: on ? 1 : 0.55 }}
            >
              {t.label}
            </span>
          </button>
