...
export interface BrandVars {
  ...
  "--brand-2": string;
  "--brand-2-ink": string;
  // Bold-theming surfaces (derived from HSL of primary)
  "--brand-header": string;
  "--brand-header-on": string;
  "--brand-wash": string;
  "--brand-tint": string;
}
...
// HSL utilities for HSL-space surface derivation (bold theming).
...
export function computeBrand(primary: string, secondary: string, base: Base): BrandVars {
  ...
  // Derive bold-theming surfaces in HSL-space so hue is always preserved.
  // Light: near-white tints — clearly hued but readable with dark text.
  // Dark:  deep rich fills — immersive team color without blinding brightness.
  let headerHex: string, washHex: string, tintHex: string;
  if (base === "light") {
    headerHex = hslToHex(h, Math.min(s, 75), 88);
    washHex   = hslToHex(h, Math.min(s, 60), 95);
    tintHex   = hslToHex(h, Math.min(s, 45), 98);
  } else {
    headerHex = hslToHex(h, Math.min(s, 80), 22);
    washHex   = hslToHex(h, Math.min(s, 70), 15);
    tintHex   = hslToHex(h, Math.min(s, 55), 11);
  }
  ...
  return {
    ...
    "--brand-soft": mixHex(primary, surface, base === "dark" ? 0.78 : 0.80),
    "--brand-2": secondary,
    "--brand-2-ink": ensureContrast(secondary, surface, 4.5),
    "--brand-header": headerHex,
    "--brand-header-on": onColor(headerHex),
    "--brand-wash": washHex,
    "--brand-tint": tintHex,
  };
}
